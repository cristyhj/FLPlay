...
var torrentStream = function (link, opts, cb) {
  ...
  if (!opts) opts = {}
  if (!opts.id) opts.id = '-UT355W-' + hat(48)
  ...
  var ontorrent = function (torrent) {
    ...
    engine.files = torrent.files.map(function (file) {
      ...
      file.offsetPiece = offsetPiece
      file.endPiece = endPiece

      /*file.deselectSelected = function () {

      }*/

      file.select = function () {
        engine.select(offsetPiece, endPiece, true)
      }

      file.selectChunkAt = function (at) {
        var atOffset = ((file.offset + at) / torrent.pieceLength) | 0
        var atOffsetPlusOne = ((atOffset + 10) > endPiece) ? endPiece : (atOffset + 10)
        engine.select(atOffset, atOffsetPlusOne, true)
      }
      file.selectChunk = function (at) {
        var atOffsetPlusOne = ((at + 10) > endPiece) ? endPiece : (at + 10)
        engine.select(at, atOffsetPlusOne, true)
      }

      file.selectEnd = function () {
        engine.select(endPiece - 5, endPiece, true)
      }

      file.selectStart = function () {
        engine.select(offsetPiece, offsetPiece + 1, true)
      }
      
      file.deselectChunkAt = function (at) {
        var atOffset = ((file.offset + at) / torrent.pieceLength) | 0
        var atOffsetPlusOne = ((atOffset + 10) > endPiece) ? endPiece : (atOffset + 10)
        engine.deselect(atOffset, atOffsetPlusOne, true)
      }
      file.deselectChunk = function (at) {
        var atOffsetPlusOne = ((at + 10) > endPiece) ? endPiece : (at + 10)
        engine.deselect(at, atOffsetPlusOne, true)
      }

      file.deselectEnd = function () {
        engine.deselect(endPiece - 5, endPiece, true)
      }

      file.deselectStart = function () {
        engine.deselect(offsetPiece, offsetPiece + 1, true)
      }
      ...
      file.createReadStream = function (opts) {
        ...
        engine.select(stream.startPiece, stream.endPiece, false, notify)
        eos(stream, function () {
          engine.deselect(stream.startPiece, stream.endPiece, false, notify)
        })

        return stream
      }

      file.createReadStreamPrio = function (opts) {
        var stream = fileStream(engine, file, opts)
        engine.stream = stream
        var notify = stream.notify.bind(stream)
    ...
    engine.clearSelection = function () {
      for (var i = 0; i < engine.selection.length; i++) {
        ...
        if (sel.priority === toNumber(false)) {
          engine.selection.splice(i, 1);
          i--;
        }
      }
      if (engine.oldSelection) {
        for (var i = 0; i < engine.oldSelection.length; i++) {
          var sel = engine.oldSelection[i]
          if (sel.priority === toNumber(false)) {
            engine.oldSelection.splice(i, 1);
            i--;
          }
        }
      }
    ...
    engine.getSelectionsString = function () {
      var str = '(';
      for (var i = 0; i < engine.selection.length; i++) {
        var sel = engine.selection[i]
        if (sel.priority === toNumber(true)) {
          str += '#'
        }
        str += '[' + sel.from + ', ' + sel.to + '], '
      }
    ...
    engine.getOldSelectionsString = function () {
      var str = '(';
      if (engine.oldSelection) {
        for (var i = 0; i < engine.oldSelection.length; i++) {
          var sel = engine.oldSelection[i]
    ...
    var pauseDownload = function () {
      engine.oldSelection = engine.selection
      engine.selection = []
      engine.oldSelection.forEach(function (sel) {
        if (sel.priority === toNumber(true)) {
          engine.selection.push(sel)
        }
      })
      engine.downloadPaused = true
      refresh()
    }

    var resumeDownload = function () {
      if (engine.oldSelection) {
        engine.selection = engine.oldSelection
        engine.oldSelection = null
        engine.downloadPaused = false
        refresh()
      }
    }

    engine.on('resumeDownload', resumeDownload)
    engine.on('pauseDownload', pauseDownload)
  ...
  engine.select = function (from, to, priority, notify) {
    ...
    if (engine.oldSelection) {
      ...
      engine.oldSelection.sort(function (a, b) {
